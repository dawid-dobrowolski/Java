...
public class EmailQueueDAO implements SenderDAO<EmailQueue> {
    ...
    public Optional<EmailQueue> get(int statusId) throws DatabaseException {
        return dbConnection.executeQuery("SELECT * FROM email_queue WHERE status_id = '%d' LIMIT 1;"
    ...
    public Long save(EmailQueue emailQueue) throws DatabaseException {
        ...
        final String statement = ("INSERT INTO email_queue (creation_date, modification_date, status_id, email_message_id, email_recipient_id) VALUES ('%s','%s','%d','%d','%d');").formatted(SenderDAO.timestampAsString(emailQueue.getCreationDate()),
    ...
    public long update(EmailQueue newEmailQueue, EmailQueue expectedEmailQueue) throws DatabaseException {
        if (newEmailQueue.getEmailQueueId() != null) {
            return dbConnection.executeUpdate(("UPDATE email_queue SET modification_date = '%s', status_id = '%d' WHERE email_queue_id = '%d' AND status_id = '%d';")
    ...
    public void delete(EmailQueue emailQueue) throws DatabaseException {
        if (emailQueue.getEmailQueueId() != null) {
            dbConnection.executeUpdate(("DELETE FROM email_queue WHERE email_queue_id = '%d';").formatted(emailQueue.getEmailQueueId()), false);
        } else {
